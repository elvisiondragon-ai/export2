...
export default function HungrylaterPaymentPageID() {
  ...
  const autoDiscountPercentage = (quantity >= 3) ? 0.2 : 0;
  const discountAmount = totalAmount * autoDiscountPercentage;
  const finalAmount = totalAmount - discountAmount;
  const isAnyDiscountApplied = autoDiscountPercentage > 0;
  ...
  useEffect(() => {
    sendCapiEvent('AddToCart', { content_name: productName, value: finalAmount, currency: 'IDR' }, `addtocart-${Date.now()}`);
  }, []);
  ...
  const handleCreatePayment = async () => {
    ...
    setLoading(true);
    sendCapiEvent('AddPaymentInfo', { content_name: productName, value: finalAmount, currency: 'IDR' }, `addpaymentinfo-${Date.now()}`);
    ...
    try {
      const { data, error } = await supabase.functions.invoke('tripay-create-payment', {
        body: {
          subscriptionType: 'hungrylater', paymentMethod: selectedPaymentMethod, userName, userEmail, phoneNumber,
          address: fullAddress, province: selectedProvince, kota, kecamatan, kodePos, amount: finalAmount, quantity,
          productName: productName + (isAnyDiscountApplied ? ' (Bundle Promo)' : ''), affiliateRef, fbc, fbp, clientIp
        }
      });
      if (data?.success) { setPaymentData(data); setShowPaymentInstructions(true); }
      else if (selectedPaymentMethod === 'BCA_MANUAL') { setPaymentData({ paymentMethod: 'BCA_MANUAL', amount: finalAmount, status: 'UNPAID', tripay_reference: `MANUAL-${Date.now()}` }); setShowPaymentInstructions(true); }
        ...
        <div className="space-y-6">
          ...
          {paymentData.paymentMethod === 'BCA_MANUAL' && (
            <Card><CardHeader><CardTitle>Transfer Manual BCA</CardTitle></CardHeader><CardContent className="space-y-4">
                ...
                <a href={`https://wa.me/62895325633487?text=${encodeURIComponent(`Halo kak, saya sudah bayar Hungrylater. Ref: ${paymentData.tripay_reference}`)}`} target="_blank" rel="noopener noreferrer" className="w-full"><Button className="w-full bg-green-500 hover:bg-green-600 text-white" size="lg"><FaWhatsapp className="mr-2" /> Konfirmasi ke CS</Button></a>
              </CardContent></Card>
          )}
    ...
    <div className="min-h-screen bg-background pb-32">
      ...
      <div className="p-6 pb-4">
        <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-4"><Button variant="ghost" size="icon" onClick={() => navigate('/')}><ArrowLeft className="w-5 h-5" /></Button><h1 className="text-2xl font-bold font-exo bg-gradient-primary bg-clip-text text-transparent">Checkout Hungrylater</h1></div>
      ...
      <div className="px-6 space-y-6">
        <Card><CardHeader><CardTitle>Rangkuman Pesanan</CardTitle></CardHeader><CardContent className="space-y-4">
            <div className="flex justify-between items-center"><Label className="text-muted-foreground">Produk</Label><span className="font-medium">{productName}</span></div>
            <div className="flex justify-center my-4"><img src={hungrylaterImage} alt="Hungrylater Product" className="w-48 h-48 object-contain" /></div>
            ...
            <div className="text-sm font-medium text-blue-600 text-center bg-blue-50 p-2 rounded-md">{quantity < 3 ? "Beli 3 untuk Diskon Otomatis & FREE Ongkir" : "✨ Bundle 3+ Berhasil: Diskon & FREE Ongkir aktif!"}</div>
            <Separator/><div className="flex justify-between items-center"><Label className="text-muted-foreground">Subtotal</Label><span className={isAnyDiscountApplied ? "line-through opacity-50" : "font-medium"}>{formatCurrency(totalAmount)}</span></div>
            ...
            <Separator/><div className="flex justify-between items-center"><div className="flex flex-col"><Label className="text-lg font-bold">Total Harga (IDR)</Label>
              ...
              </div><div className="text-right"><span className={`font-bold text-xl text-primary`}>{formatCurrency(finalAmount)}</span></div>
            </div>
          </CardContent></Card>
        ...
        <div className="bg-slate-900 p-6 md:p-8 rounded-3xl space-y-6 shadow-2xl border border-slate-800">
          ...
          <ul className="space-y-4">
            <li className="flex items-start gap-4"><div className="mt-1 bg-primary/20 p-1.5 rounded-full"><Zap className="h-4 w-4 text-primary" /></div><p className="text-base text-slate-200 font-medium leading-relaxed"><b className="text-white">1 Box Hungrylater Weight Control</b> (Konsumsi max 1x sehari untuk hasil optimal).</p></li>
            <li className="flex items-start gap-4"><div className="mt-1 bg-primary/20 p-1.5 rounded-full"><BookOpen className="h-4 w-4 text-primary" /></div><p className="text-base text-slate-200 font-medium leading-relaxed"><b className="text-white">Exclusive Weight Control Guidance Booklet</b> (Panduan lengkap kontrol nafsu makan).</p></li>
            <li className="flex items-start gap-4"><div className="mt-1 bg-primary/20 p-1.5 rounded-full"><Music className="h-4 w-4 text-primary" /></div><p className="text-base text-slate-200 font-medium leading-relaxed"><b className="text-white">Appetite Control Sync Hypnosis Audio</b> (Digital Access yang dikirimkan saat barang sampai).</p></li>
          </ul>
          <div className="pt-6 space-y-2 border-t border-slate-800"><p className="text-sm font-black text-primary flex items-center gap-2 uppercase tracking-tighter"><Zap className="h-4 w-4" /> High Technology Synchronization</p><p className="text-xs leading-relaxed text-slate-400 font-medium italic">"Minum Hungrylater sambil mendengarkan audio hypnosis appetite control akan membantu tubuh masuk ke fase deep relaxation, memastikan sinyal kenyang tersampaikan ke otak lebih efektif."</p></div>
        </div>
        ...
        <div className="fixed bottom-20 left-6 right-6">
          <Button onClick={handleCreatePayment} disabled={loading} className="w-full h-14 text-xl font-bold shadow-xl transition-all active:scale-95 bg-gradient-to-r from-[#BF953F] via-[#FCF6BA] to-[#B38728] text-amber-900 border-none hover:opacity-90">
            ...
            {loading ? 'Memproses...' : `Bayar Sekarang (${formatCurrency(finalAmount)})`}
          </Button>
        </div>
