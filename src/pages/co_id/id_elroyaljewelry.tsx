...
export default function JewelryPaymentPageID() {
  ...
  const baseProductName = 'Jewelry eL Royale';
  const variants = { 'Silver': 3500000, 'Gold': 7000000 };

  const [quantities, setQuantities] = useState({ 'Silver': 1, 'Gold': 0 });
  ...
  const handleCreatePayment = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('tripay-create-payment', {
        body: {
          subscriptionType: 'jewelry', paymentMethod: selectedPaymentMethod, userName, userEmail, phoneNumber,
        ...
        <div className="space-y-6">
          ...
          {paymentData.paymentMethod === 'BCA_MANUAL' && (
            <Card><CardHeader><CardTitle>Transfer Manual BCA</CardTitle></CardHeader><CardContent className="space-y-4">
                ...
                <a href={`https://wa.me/62895325633487?text=${encodeURIComponent(`Sudah bayar Jewelry. Ref: ${paymentData.tripay_reference}`)}`} target="_blank" rel="noopener noreferrer" className="w-full"><Button className="w-full bg-green-500 hover:bg-green-600 text-white" size="lg"><FaWhatsapp className="mr-2" /> Konfirmasi CS</Button></a>
              </CardContent></Card>
          )}
    ...
    <div className="min-h-screen bg-background pb-32">
      ...
      <div className="p-6 pb-4">
        <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-4"><Button variant="ghost" size="icon" onClick={() => navigate('/')}><ArrowLeft className="w-5 h-5" /></Button><h1 className="text-2xl font-bold font-exo bg-gradient-primary bg-clip-text text-transparent">Checkout Jewelry</h1></div>
      ...
      <div className="px-6 space-y-6">
        <Card><CardHeader><CardTitle>Rangkuman Pesanan</CardTitle></CardHeader><CardContent className="space-y-4">
            <div className="flex justify-center mb-4"><img src={jewelImage} alt="Jewelry" className="w-48 h-48 object-contain" /></div>
            <Separator />
        ...
        <div className="bg-slate-900 p-6 md:p-8 rounded-3xl space-y-6 shadow-2xl border border-slate-800">
          ...
          <ul className="space-y-4">
            <li className="flex items-start gap-4"><div className="mt-1 bg-primary/20 p-1.5 rounded-full"><Zap className="h-4 w-4 text-primary" /></div><p className="text-base text-slate-200 font-medium leading-relaxed"><b className="text-white">Premium Custom Jewelry</b> (Handcrafted dengan material berkualitas tinggi).</p></li>
            <li className="flex items-start gap-4"><div className="mt-1 bg-primary/20 p-1.5 rounded-full"><BookOpen className="h-4 w-4 text-primary" /></div><p className="text-base text-slate-200 font-medium leading-relaxed"><b className="text-white">Exclusive Jewelry Care Booklet</b> (Panduan perawatan agar perhiasan tetap berkilau).</p></li>
            <li className="flex items-start gap-4"><div className="mt-1 bg-primary/20 p-1.5 rounded-full"><Music className="h-4 w-4 text-primary" /></div><p className="text-base text-slate-200 font-medium leading-relaxed"><b className="text-white">Self-Confidence Sync Hypnosis Audio</b> (Digital Access yang dikirimkan saat barang sampai).</p></li>
          </ul>
          <div className="pt-6 space-y-2 border-t border-slate-800"><p className="text-sm font-black text-primary flex items-center gap-2 uppercase tracking-tighter"><Zap className="h-4 w-4" /> High Technology Synchronization</p><p className="text-xs leading-relaxed text-slate-400 font-medium italic">"Mengenakan perhiasan eL Royale sambil mendengarkan audio hypnosis self-confidence akan mensinkronisasi energi positif ke dalam diri Anda, memancarkan aura kecantikan dan kepercayaan diri yang maksimal."</p></div>
        </div>
        ...
        <div className="fixed bottom-20 left-6 right-6">
          <Button onClick={handleCreatePayment} disabled={loading} className="w-full h-14 text-xl font-bold shadow-xl transition-all active:scale-95 bg-gradient-to-r from-[#BF953F] via-[#FCF6BA] to-[#B38728] text-amber-950 border-none hover:opacity-90">
        ...
        <footer className="mt-8 px-6 text-center pb-10"><div className="pt-6 border-t border-slate-200"><p className="text-slate-500 text-xs font-semibold">© 2026 eL Royal Jewelry. All Rights Reserved.</p></div></footer>
      </div><WhatsAppButton />
    </div>
  );
}
